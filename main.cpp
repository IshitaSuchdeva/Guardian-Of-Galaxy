		...
		if (mouseX <= (xStone[i] / 2 + 20) && mouseX >= (xStone[i] / 2 - 20) && mouseY >= (yStone[i] / 2 - 20) && mouseY <= (yStone[i] / 2 + 20) && mButtonPressed) {
			if (stoneAlive[i]) {   // If alive, kill the stone
				...
				if (Score % 10 == 0) {    // Check if the score is a multiple of 10
					...
					stoneTranslationSpeed += 3;    // Adjust any game parameters related to level increase
				}
				if (Score % 3 == 0) {
					stoneTranslationSpeed += 1;     // Rate of increase of game speed
				}
...
void InstructionsScreenDisplay()
{
	...
	displayRasterText(-900, -200, 0.4, (char*)"You Get 1 point for shooting each objet and 50 points for completing each lvl ");
